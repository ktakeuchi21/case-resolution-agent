...
export function currentOrientation(knowledge,now=Date.now()) {
 const expiry=knowledge?.kind==='upload'?knowledge.sources?.[0]?.expiresAt:null;
 if(!expiry||Date.parse(expiry)>now)return knowledge;
 return {...knowledge,available:false,name:'Temporary knowledge unavailable',status:'Temporary knowledge expired',coverage:[],sources:[],revision:knowledge.revision+':expired',summary:'This temporary document has expired. Choose new knowledge to continue.'};
}
export function leaveLaunchpad(view,previous) {
 if(!previous)return previous;
 const hasMessage=view.agent.entries.some(e=>e.operation!=='new_conversation'&&e.conversationId===view.agent.activeConversationId&&e.knowledge?.key===previous.knowledgeKey);
 return hasMessage?{...previous,mode:'compact',announcement:''}:previous;
}
export function suggestedPrompts({role,knowledge,workflow,settings}) {
 ...
 if(!knowledge.coverage?.includes('Missing-document process')&&workflow.dependency!=='resolved') {
  const gaps={office:['Which case information is available?','What knowledge is missing for this request?','Why is the next step waiting on knowledge review?','Who can review the knowledge gap?'],manager:['Summarize the knowledge gap.','Which next steps need current governing evidence?','Who owns the knowledge review?','What is the next checkpoint?'],supervisor:['Brief me on current knowledge and risk.','Which authority is missing?','What must be reviewed before continuing?','What is waiting on human review?'],knowledge_reviewer:['Which sources are currently eligible?','Why are some selected sources unavailable?','What would restore governed support?','Why can’t a sandbox upload restore authority?']};
  return gaps[role]??gaps.office;
 }
...
export function launchpadState(view,previous=null,{expanded=null,submitting=false}={}) {
 const k=currentOrientation(view.agent.orientation), entries=view.agent.entries.filter(e=>e.operation!=='new_conversation');
...
export function renderContextBar(view,state) {
 const k=currentOrientation(view.agent.orientation);
...
export function renderLaunchpad(view,state,sending=false) {
 const k=currentOrientation(view.agent.orientation);if(!k)return '<p role="status">Refresh to load the current knowledge context.</p>';
