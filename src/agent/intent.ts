...
export function interpretMessage(message: z.infer<typeof ConversationRequest>, history: AgentResponse[], settings: AgentSettings, generation: 'model' | 'evidence') {
 ...
 const interaction = /\b(?:record|log|summari[sz]e)\b[\s\S]*\b(?:transcript|call note|interaction|message|call)\b[\s\S]*[:\n]/i.test(text);
 ...
 if (refinement && !product) interpretation.intent = 'clarification';
