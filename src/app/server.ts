...
async function body(req:IncomingMessage){
 ...
 let length=0;const parts:Buffer[]=[];for await(const part of req){length+=part.length;if(length>bodyLimit)throw new HttpError(413,'Upload/request too large. Maximum file size is 8 KB.');parts.push(part);}
...
export function createApplicationServer(options:{db?:Database;sessionDb?:Database;webRoot?:string}={}){
 ...
 const server=createServer(async(req,res)=>{
  ...
  try{
   ...
   if(url.pathname.startsWith('/api/')){
    ...
    sessions.verifyCsrf(req,s);await sessions.limit('write.'+s.token_hash,30,60);await sessions.limit('attempts.'+s.token_hash,150,14400);const input=await body(req);
   ...
   const mime:Record<string,string>={'.html':'text/html; charset=utf-8','.js':'text/javascript; charset=utf-8','.css':'text/css; charset=utf-8','.svg':'image/svg+xml','.png':'image/png','.webp':'image/webp','.woff2':'font/woff2'};
 ...
 return {server,db,sessionDb,app,sessions,close:async()=>{await new Promise<void>((r,j)=>server.close(e=>e?j(e):r()));await db.close();await sessionDb.close();}};
}
