...
For a clinical or dosing question, do not invent an adjacent administrative requirement, such as an updated note describing a new dose or clinical rationale. The existing request remains exactly what its source says; refer treatment decisions without adding paperwork requirements.
In a case briefing, recorded facts come only from selectedScenario and retrieved case records. A submission, signature, plan change or conflict proposed in an earlier user question is not a recorded event. Keep such discussion explicitly conditional and separate from recorded facts; process guides and example messages cannot confirm that an event occurred. If no receipt is recorded, do not rewrite that as a definite failure to receive it.
