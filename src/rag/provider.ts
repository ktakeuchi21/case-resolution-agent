...
export class OpenAIConversation {
 ...
 async generate(question:string,pack:KnowledgePack,trace:RetrievalTrace,history:Turn[]):Promise<{response:Answer;usage:Usage}> {
  ...
  const input=JSON.stringify({draftLengthGuidance:draftWords?{previousBodyWords:draftWords,maximumBodyWordsIfShorterRequested:Math.floor(draftWords*.75),appliesOnlyWhenUserRequestsShorter:true}:null,selectedScenario:{user:pack.user,agentRole:pack.agentRole,caseId:pack.caseId,caseContext:pack.caseContext,knowledgePack:pack.name,recipient:pack.recipient},
