...
test('conversation fast path: notice, inferred products, knowledge changes, memory and temporary deletion',async()=>{
 ...
 try{
  ...
  assert.equal(initial.agent.orientation.available,true);assert(initial.agent.orientation.summary.includes('administrative communication guidance'));assert.equal(initial.agent.orientation.assignment.caseId,'DEMO-101');
  ...
  const orientationRevision=(await req('state')).agent.orientation.revision;
  const roleChanged=await req('action',{action:'role',role:'supervisor',idempotencyKey:randomUUID()});assert.equal(roleChanged.role,'supervisor');assert.equal(hash(roleChanged.workflow),baseline);assert.equal(roleChanged.agent.orientation.revision,orientationRevision);assert.equal(hash(roleChanged.agent.entries.find((e:{id:string})=>e.id===answer.id)),hash(answer));
  ...
  const sandboxOrientation=(await req('state')).agent.orientation;assert.equal(sandboxOrientation.name,upload.metadata.sourceName);assert.equal(sandboxOrientation.assignment,null);assert(sandboxOrientation.boundary.includes('cannot authorize'));assert.equal(sandboxOrientation.sources.length,1);
  ...
  const deleted=await req('state');assert.equal(deleted.agent.orientation.available,false);assert.equal(deleted.agent.orientation.sources.length,0);assert(deleted.agent.orientation.summary.includes('expired'));assert.equal(deleted.agent.knowledge,null);assert(!deleted.agent.feedback.some((f:{entry_id:string})=>f.entry_id===sandbox.id));assert(!deleted.agent.entries.some((e:{id:string})=>e.id===sandbox.id));assert(deleted.agent.entries.some((e:{id:string})=>e.id===answer.id));assert.equal(hash(deleted.workflow),baseline);
  ...
  const beforeNew=await req('state');await req('agent',{operation:'new_conversation',synthetic:true,idempotencyKey:randomUUID()});const afterNew=await req('state');assert.notEqual(afterNew.agent.activeConversationId,beforeNew.agent.activeConversationId);assert.equal(hash(afterNew.workflow),baseline);assert.equal(hash(afterNew.agent.entries.find((e:{id:string})=>e.id===answer.id)),hash(answer));
  const expiring=await req('studio-upload',{name:'expiring.txt',base64:Buffer.from('Synthetic administrative example.').toString('base64'),synthetic:true});await req('agent-preferences',{action:'select',selection:{kind:'upload',uploadId:expiring.id}});await service.sessionDb.pool.query("UPDATE portfolio.studio_uploads SET expires_at=clock_timestamp()-interval '1 second' WHERE workspace=$1 AND id=$2",[initial.demoId,expiring.id]);const expired=await req('state');assert.equal(expired.agent.orientation.available,false);assert.deepEqual(expired.agent.orientation.sources,[]);
