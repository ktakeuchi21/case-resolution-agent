...
test('unknown, scalar, inherited, oversized and malformed contexts cannot become diagnostic paths',()=>{
 ...
 for(const context of ["'properties', 'absent_node'","'properties', 'answer', 'type'","'__proto__'","'properties'; code()","'properties', x",Array(34).fill("'properties'").join(','),"'"+'x'.repeat(2100)+"'"]){
  assert.equal(schemaRejectionCode(`In context=(${context}), anyOf is not allowed.`,schema),'PROVIDER_SCHEMA_REJECTED_ANYOF_NOT_ALLOWED_CONTEXT'+(context.includes('properties')?'_PROPERTIES':''));
 }
});
...
test('expanded error locations resolve local shared definitions without exposing their paths',()=>{
 ...
 for(const ref of ['https://example.invalid/schema','#/$defs/absent','#/$defs/loop']){
  ...
  assert.equal(schemaRejectionCode("In context=('properties', 'answer', 'items'), anyOf is not allowed",unsafe),'PROVIDER_SCHEMA_REJECTED_ANYOF_ITEMS_NOT_ALLOWED_CONTEXT_PROPERTIES');
 }
});
...
test('the actual provider transport associates a bounded rejection with its own submitted schema',async()=>{
 ...
 await assert.rejects(provider.interpret(input),error=>error instanceof AgentProviderFailure&&/^PROVIDER_SCHEMA_REJECTED_ANYOF_NOT_ALLOWED_CONTEXT_PROPERTIES_AT_NODE_\d+_SCHEMA_[a-f0-9]{12}$/.test(error.code)&&error.code.endsWith(fingerprint)&&!error.message.includes('private-provider-marker'));
