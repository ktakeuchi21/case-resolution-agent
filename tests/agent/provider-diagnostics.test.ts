...
test('schema location diagnostics contain only a node index and request-schema fingerprint',()=>{
 ...
 const prefix='PROVIDER_SCHEMA_REJECTED_ANYOF_NOT_ALLOWED_FORMAT',message="Invalid schema for response_format: In context=('properties', 'private_marker'), anyOf is not allowed. private document content";
 ...
 assert.equal(schemaRejectionCode('In context=(), anyOf is not allowed',schema),'PROVIDER_SCHEMA_REJECTED_ANYOF_NOT_ALLOWED_AT_NODE_0_SCHEMA_'+textHash(JSON.stringify(schema)).slice(0,12));
});
...
test('unknown, scalar, inherited, oversized and malformed contexts cannot become diagnostic paths',()=>{
 ...
 for(const context of ["'properties', 'absent_node'","'properties', 'answer', 'type'","'__proto__'","'properties'; code()","'properties', x",Array(34).fill("'properties'").join(','),"'"+'x'.repeat(2100)+"'"]){
  assert.equal(schemaRejectionCode(`In context=(${context}), anyOf is not allowed.`,schema),'PROVIDER_SCHEMA_REJECTED_ANYOF_NOT_ALLOWED');
 }
});
...
test('array positions identify separate schema nodes without retaining their field names',()=>{
 ...
 assert.equal(code,'PROVIDER_SCHEMA_REJECTED_ANYOF_NOT_ALLOWED_AT_NODE_3_SCHEMA_'+textHash(JSON.stringify(schema)).slice(0,12));
});
...
test('the actual provider transport associates a bounded rejection with its own submitted schema',async()=>{
 ...
 await assert.rejects(provider.interpret(input),error=>error instanceof AgentProviderFailure&&/^PROVIDER_SCHEMA_REJECTED_ANYOF_NOT_ALLOWED_AT_NODE_\d+_SCHEMA_[a-f0-9]{12}$/.test(error.code)&&error.code.endsWith(fingerprint)&&!error.message.includes('private-provider-marker'));
