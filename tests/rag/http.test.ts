...
test('catalog contains expanded synthetic source passages and no infrastructure/private key fields',async()=>{const r=await request('catalog');assert.equal(r.status,200);assert.equal(r.body.packs.length,3);assert.equal(r.body.documents.length,42);assert.equal(r.body.passages.length,110);assert(r.body.packs.every((p:any)=>p.version==='1.1'));assert(r.body.passages.every((p:any)=>p.synthetic));assert(!JSON.stringify(r.body).includes('OPENAI_API_KEY'));});
