...
The provider writes complete ordered prose segments for the answer, optional rationale and recipient subject/body. Each segment contains its text and exact supporting references; nonfactual greetings and acknowledgments may be marked style. The app preserves the model-authored strings, inserts a space at otherwise fused segment boundaries, and derives claim locations from the unchanged segment text. It adds no words or templates. This avoids asking the model to reproduce its prose a second time in a separate ledger. The persisted proposed turn retains full text, audience/channel/tone/purpose, the derived claim ledger, uncertainties and an operational descriptor that must remain null. Both raw segmented prose and the materialized turn are audited. Audience/channel/tone are constrained to the interpreted values; ordinary questions cannot acquire an unsolicited work product, and artifact requests require the work-product object. The reviewer uses objects keyed by exactly the current claim IDs and output slots; these materialize into the compatible stored review arrays without dropping or merging verdicts. Every claim must occur verbatim in its declared output slot and cite an exact current passage or typed fact. Source citations must match eligible, supporting, hash-verified evidence. The production schema pairs each reference with exact current quote choices, including bounded exact slices for long passages, preventing punctuation changes or quotes attached to another reference before runtime validation. Conversation facts can only support explicitly unverified uncertainty. The independent review must cover every nonempty answer, rationale, subject and body, including factual statements improperly marked as style. Current workflow rationale and the payer boundary are explicit typed facts. A separate deterministic guard rejects the documentation-dependent payer timing claims observed during live review. A missing slot, unsupported claim, quality rejection or authority violation pauses the entire answer.
...
The later live run exposed two additional holes: material text labeled style bypassed the ledger, and whole-turn review borrowed uncited conversation context to approve factual claims. Uncited style is now limited to neutral greetings, thanks and unsent acknowledgments, with whitespace variants. The model still writes every substantive answer, subject and body; there are no whole-answer or work-product templates. All other prose must become a cited claim. Recording/recalling a referenced interaction and case summaries containing a report require a visible unverified label and a conversation-backed uncertainty claim.

The provider reviewer receives the proposed turn and a claim-keyed projection of the exact cited quotes with their authority labels. Uncited history, active-artifact prose, and interpretation notes are omitted from entailment review. The actual query and interpreted audience/channel/intent remain for request-quality checks. Structured request IDs must occur in their claim’s quote or reference. Explicit active-artifact context is supplied to interpretation, and source titles to composition. These controls narrow known failure paths; model entailment review remains fallible and needs fresh live qualification.

Contextual schemas use shared definitions without sibling metadata on references, reducing a representative pretty-printed schema from 108,336 to 14,437 bytes. The provider response still has its 100 KB bound. Request-error envelopes are inspected only within 8 KB and reduced to fixed diagnostic codes; their untrusted messages are never retained. The frozen browser runner now stops immediately on a provider-contract/budget failure instead of submitting remaining prompts.
