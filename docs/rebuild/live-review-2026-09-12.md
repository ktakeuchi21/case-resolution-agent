...
Prepared prompt correction deployed successfully as source **`691632b0590d46a35c4ba2d716e687ecea682977`**, Render deployment **`dep-dail5geq1p3s7394f9rg`**. Read-only post-deployment checks confirm GPT-5 mini is active and the durable counter remains 250. The additional-allowance setting was verified absent in Render. No live generation was attempted after exhaustion. The production build, 20 RAG tests, type check and hygiene check pass (401 text files, 134 content-addressed artifacts); the new prompt remains live-unqualified pending approved capacity or the next UTC day's ordinary allowance.

## Approved continuation and source-reference formatting

The user explicitly approved the 150 additional requests. Environment deployment `dep-dail86mk1f9s738lsdvg` activated the 400 total for the existing acceptance date, retaining the counter at 250 and GPT-5 mini. The compact-v1 critical run completed eight turns, then stopped at fulfillment-01 after two `INVALID_CITATION_MAPPING` attempts. Its immutable artifact is `critical-8275b508bd4c849649a8ec3d330717de4b2c657efcb4acc383ed77b44f74e2ba.json`; 11 generation and 6 embedding requests cost $0.01686150. The revised receipt answer now accurately says receipt is not recorded, and access-09 returns the requested separate summary work product. All eight completed turns retain exact, eligible citations. This partial run does not qualify the complete gate.

Review of the formatting path found that ordinary responses were forced to repeat every ID in the model's returned citations array, including retrieved IDs not actually used inline. The citation formatter now derives the ordinary answer's source list from its actual inline references, numbers them by first appearance, and leaves the prose untouched. Every returned ID is still checked against retrieval before normalization; unknown IDs, duplicate returned IDs, out-of-range numeric markers and a nonempty source list without any inline reference still fail. Work products retain their separate validated source list. The repair instruction no longer demands that brief work-product introductions repeat every source or that an answer expand solely to mention all retrieved passages. These are citation-presentation changes, not factual rewriting, added model calls or changes to frozen semantic expectations.

The next run targets the six frozen fulfillment cases in the existing critical subset. The original eight completed rows are retained; the complete unchanged 36-turn suite remains required after the focused regression succeeds. Fresh full-suite visitors will preserve the ordinary 40-request session ceiling.

## Completed headless qualification and new interface

The retained full GPT-5 mini run completed all 36 frozen turns. The detailed [conversation review](conversation-review-2026-09-12.md) separates measured retrieval/citation results, two original partial semantic passes, targeted wording corrections and the passing six-step knowledge-switch supplement. The whole-answer source-list contract is now the final attribution design: every source ID must be retrieved, but prose need not repeat an inline marker for every source. The source-list supplement needed no repair.

After those headless checks, the new landing, direct scenario launch, chat/email thread, exact-source disclosures, Knowledge Pack browser and evaluation page were implemented. [Interface and browser verification](interface-and-browser.md) records the decisions, local UI replay and the subsequent live public acceptance. Optional upload is deferred. Earlier entries above are chronological history and do not describe the completed interface.
