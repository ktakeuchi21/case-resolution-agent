# Approval proposal: bounded live-acceptance allowance
...
Proposed action, awaiting explicit approval:
...
Implementation is in `src/rag/service.ts` (`providerDailyCeiling`). No exception is active without the exact UTC date in server configuration. Local tests verify the 100 default, 250 on the configured date, and automatic return to 100 the next day. No environment mutation or provider-cap increase has been made.
