...
## September 9, 2026 — Audit return and in-page expiry explicitly

A passing test that manually hid context before navigating did not cover leaving expanded context. Strengthen the acceptance trace and collapse existing orientation on departure, preserving change detection and history. Enforce temporary upload expiry in presentation with one local deadline and visibility restoration, in addition to unchanged server enforcement; remove expired temporary derivatives from rendered memory. Knowledge-gap prompts replace drafting suggestions when current authoritative process coverage disappears. These changes only reduce stale presentation, never grant knowledge or action authority. See the follow-up audit in `conversation-launchpad.md` for the reproduced failure and corrected checks.

## September 9, 2026 — Preserve grounding while locating provider schema failures

The previous broad error category does not identify which nested schema was rejected. Validate a bounded provider-reported context against the submitted schema and retain only its structural node index and schema hash prefix. Raw messages, path names and document values remain excluded. Keep the same schema, neutral-style restriction, validators, model and cap; do not infer compatibility from local parsing. Strengthen the real application test to inspect provider inputs across sample/pack/upload switches, exact retained history and fresh evidence. The full launchpad requirement audit remains open at the normal live-response gate.
