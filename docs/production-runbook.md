...
The September 9 static deployment is a review candidate; its rewrite buffers streamed status messages, so the public-link rollout is held. Keep https://case-resolution-agent.onrender.com as the public fallback. See [static frontend verification and current rollout gate](static-frontend.md) for exact service/deployment revisions, configuration, measurements and the required transport check. Release matching frontend/backend revisions manually and repeat cookie/CSRF/isolation/upload/download/stream checks before promoting any changed routing arrangement.
