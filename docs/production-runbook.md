...
`render.yaml` defines one free Docker web service with automatic deployment disabled, the hosted database profile and CA path, a manual runtime database secret and `/healthz`. The origin may use Render's validated platform-supplied URL. It provisions no database, paid worker, custom domain or preview fleet. The deployed service selects Oregon, Free, `NODE_ENV=production`, `/healthz` and automatic deployments Off. Its verified origin is https://case-resolution-agent.onrender.com. Confirm free-resource/overage settings and service-name ownership before applying it. During first setup `sync: false` fields prompt for values; later additions require manual environment management. [Render Blueprint reference](https://render.com/docs/blueprint-spec)
