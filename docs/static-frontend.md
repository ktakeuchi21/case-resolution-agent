...
September 9 routing adjustment: the owner approved deployment of a static overview that waits for readiness and then opens the workspace at the existing backend address. `web/hosting.js` fixes both HTTPS addresses and accepts only known routes/scenarios; it transfers no session token, CSRF token or case identifier. A public start timestamp preserves the 90-second deadline across navigation and is removed from the address on arrival. Browser Back returns to the overview. All session restoration, case creation, uploads and streamed conversation stay at the backend origin. Local handoff verification passes 23/23 in `static-handoff-f5488319b2a585973a7aec61e788d72f2592470c2f7aced35e822cc2084de88b.json`. Deployment and final public verification of this adjustment are underway; the sections below retain the initial rewrite-candidate findings.
