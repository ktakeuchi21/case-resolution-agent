...
Public entry: [Open Pathway Agent](https://case-resolution-frontend.onrender.com). The free Render Static Site serves only `dist/web` through the CDN. Launch waits for backend readiness, then opens the requested workspace at https://case-resolution-agent.onrender.com. The owner approved this routing adjustment after the initial static rewrite buffered streaming updates. All **31 deployed transport checks passed** on this arrangement. The backend address also remains a working standalone fallback.

## Current verified runtime

Both services are Live on `0aa0143ce0ffa2feb0e98440ccae50915db86b65`: backend `dep-dahg60fqj5pc73aiq810` and static `dep-dahg64n40ujc73aj0pdg`. New workspace entry succeeds with the owner-approved lifetime limit of 128. The default suggested question returns a validated live answer. The final audit/evaluation release uses the same runtime; its exact deployment record is maintained in [the production runbook](production-runbook.md).

The full verification passes 285 automated tests, the public Launchpad trace passes 28/28, and public transport/security checks pass 31/31. The separate multi-turn conversation comparison remains incomplete and unsuccessful; it is shown truthfully in the visible evaluation report. See [the acceptance audit](launchpad-acceptance-audit.md) for evidence and scope.
