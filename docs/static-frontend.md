...
September 10: both services are now Live on diagnostic revision `ef7dc85dd41e0f19bab0ad636c0a9c6758268940`. The CDN landing remains available and backend health/assets pass, but **new workspaces are currently unavailable at the deliberate 64-workspace lifetime capacity**. The model budget has reset (2/100 used); its independently confirmed schema rejection is still unresolved. A bounded capacity increase is prepared for owner approval, not applied. See `launchpad-acceptance-audit.md` for the current evidence and exact release IDs. The previous release and timings below are historical.

The preceding `4cc2cd7fbf7dbc7b88197d4127e50ae9d0847d61` release added privacy-preserving provider-error diagnostics, stronger knowledge-boundary tests and the updated evaluation report; it did not change hosting, request schemas or successful conversation behavior. Its CDN checks passed 16/16, backend health/assets passed, and manual Launch → sample orientation passed. The earlier `a63de72` interface release fixed return navigation and in-page upload expiry and passed 31/31 public transport/security checks; its desktop first contentful paint measured 176 ms. That timing is a retained historical sample. See `conversation-launchpad.md` for those release records and `launchpad-acceptance-audit.md` for the current gate.
