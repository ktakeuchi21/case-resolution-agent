...
## Contextual conversation work

Read [contextual-worker.md](contextual-worker.md), then `src/agent/turn-contract.ts`, `conversation-provider.ts`, `contextual.ts` and `service.ts`. Run `npm run typecheck`, `npm run test:agent` and the application suite on a migrated local verification database. `node scripts/verify-worker.ts` runs the full release suite and checks prior artifacts for changes. Migration 013 must precede deployment; `/healthz` checks the feedback table as well as existing conversation tables.

For explicit offline UI checks: `node scripts/db/with-verification.ts serve-contextual-fixture` starts localhost port 3013 with predetermined provider outputs. Use `python3 scripts/browser/run-contextual.py SESSION fixture ui` through the established native Playwright CLI. This test-only provider is never selected by the production runtime. Its results measure UI/API contracts, not language quality.

For live paired core evaluation, use a fresh browser visitor and `python3 scripts/browser/run-contextual.py SESSION baseline core` before deployment or `... redesigned core` after deployment. The frozen prompts and rubric are hash-checked. `... redesigned boundary` uses a separate synthetic scenario group and performs only workspace-scoped governance actions. Check the durable daily/session budget before live execution; never clear it or silently raise it. Retain every failure and exact source revision. Regeneration must create a new evidence record; transport retry must return the existing idempotent response.
