...
const args = ['serve','serve-built','serve-contextual-fixture'].includes(command) ? [command==='serve-contextual-fixture'?'scripts/browser/local-contextual-server.ts':command==='serve-built'?'dist/src/app/server.js':'src/app/server.ts'] : command === 'migrate' ? ['scripts/db/cli.ts', 'migrate'] : ['--test', '--test-concurrency=1', ...readdirSync(`tests/${command}`).filter(f => f.endsWith('.test.ts')).map(f => `tests/${command}/${f}`)];
if (!['app', 'workflow', 'postgres', 'serve', 'serve-built', 'serve-contextual-fixture', 'migrate'].includes(command)) throw new Error('Choose app, workflow, postgres, serve, serve-built or migrate.');
