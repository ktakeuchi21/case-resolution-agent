...
try{
 ...
 if(!live)await page.clock.install();
 ...
 await page.getByRole('link',{name:'Change knowledge',exact:true}).click();await page.getByRole('button',{name:'Use sample knowledge →',exact:true}).click();await page.locator('#agent-text').waitFor();check('F: returning to same conversation stays compact',!await page.locator('#conversation-launchpad').isVisible()&&await page.locator('.agent-turn').count()===1);
 ...
 if(!live){const remaining=await page.evaluate(expiry=>Math.max(1,Date.parse(expiry)-Date.now()+20),upload.expiresAt);await page.clock.fastForward(remaining);await page.clock.runFor(30);report.expiry={expiresAt:upload.expiresAt,...await page.evaluate(()=>({now:Date.now(),knowledge:document.querySelector('.launchpad-context')?.innerText,disabled:document.querySelector('#agent-form button[type=submit]')?.disabled,prompts:document.querySelectorAll('[data-agent-suggestion]').length}))};check('Upload expires while chat remains open without another request',await page.getByRole('link',{name:'Choose new knowledge',exact:true}).isVisible()&&await page.locator('#agent-form button[type=submit]').isDisabled()&&await page.locator('[data-agent-suggestion]').count()===0);await page.clock.setSystemTime(new Date());await page.clock.resume();}
