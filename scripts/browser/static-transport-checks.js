async page => {
 const base='https://case-resolution-frontend.onrender.com',results=[],errors=[],measurements=[];
 const check=(name,pass,detail)=>{results.push({name,pass:!!pass,...(detail===undefined?{}:{detail})});if(!pass&&name!=='proxy delivers a status chunk before the final result')throw new Error(name);};
 ...
 try{
  ...
  await page.getByRole('button',{name:'Launch guided demo',exact:true}).first().click();await page.locator('.knowledge-picker').waitFor({timeout:95000});
  const session=await call(page,'/api/session');csrf=session.body.csrf;
  check('frontend launch restores session and creates one case',session.status===200&&session.body.data?.demoId&&requests.filter(p=>p==='/api/demo').length===1&&session.cache?.includes('no-store'));
  const cookies=await page.context().cookies(base),cookie=cookies.find(c=>c.name==='pathway_session');
  check('first-party session cookie retains Secure HttpOnly and Strict',cookie?.secure&&cookie?.httpOnly&&cookie?.sameSite==='Strict'&&cookie?.domain==='case-resolution-frontend.onrender.com',{secure:cookie?.secure,httpOnly:cookie?.httpOnly,sameSite:cookie?.sameSite,domain:cookie?.domain});
  const originalId=session.body.data.demoId,baseline=JSON.stringify(session.body.data.workflow);
  for(const token of [undefined,'invalid-csrf']){const denied=await call(page,'/api/demo',{scenario:'exception'},token);check((token?'incorrect':'missing')+' CSRF rejected through rewrite',denied.status===403&&denied.cache?.includes('no-store'));}
  const forbidden=await page.context().request.post(base+'/api/demo',{headers:{Origin:'https://untrusted.example','Content-Type':'application/json','x-csrf-token':csrf},data:{scenario:'exception'}});
  check('forged origin rejected through rewrite',forbidden.status()===403);
  const crossSite=await page.context().request.post(base+'/api/demo',{headers:{Origin:base,'Sec-Fetch-Site':'cross-site','Content-Type':'application/json','x-csrf-token':csrf},data:{scenario:'exception'}});
  check('cross-site metadata rejected through rewrite',crossSite.status()===403);
  ...
  other=await browser.newContext();const second=await other.newPage();await second.goto(base);const sessionB=await call(second,'/api/session');
  ...
  upload=uploaded.body;check('synthetic upload parses through rewrite without caching',uploaded.status===200&&upload.parsed?.passages.length>0&&upload.status==='parsed'&&uploaded.cache?.includes('no-store'));
  ...
  check('NDJSON stages and final response arrive uncached through rewrite',streamed.status===200&&streamed.type?.includes('application/x-ndjson')&&streamed.cache?.includes('no-store')&&streamed.stages.includes('retrieving')&&streamed.hasResult&&!streamed.error,streamed);
  check('proxy delivers a status chunk before the final result',streamed.chunks.some((c,i)=>c.stage&&!c.result&&streamed.chunks.slice(i+1).some(x=>x.result)));
 ...
 return {schema:'pathway-static-transport-browser-v1',timestamp:new Date().toISOString(),environment:'Public Render static frontend and same-origin rewrites; isolated synthetic sessions; explicit evidence mode with retained vectors; no live model-quality claim',url:base,revision:'969c46cee3a5912acb39c441f7ba7fd6d2a3ac22',measurements,results,errors,...(error?{error}:{})};
}
