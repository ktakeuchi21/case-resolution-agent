async page => {
 ...
 try{
  ...
  await route('studio');await page.locator('[data-source="K-PA"]').click();await page.locator('#source-version').selectOption('K-PA.v2');await page.getByRole('combobox',{name:'Demo role',exact:true}).selectOption('knowledge_reviewer');await ready();await page.getByRole('button',{name:'Retire this version',exact:true}).click();await page.getByRole('button',{name:'Confirm change',exact:true}).click();await page.getByRole('button',{name:'Retirement recorded',exact:true}).waitFor();check('J intended version retirement is committed',(await state()).knowledge.events.some(e=>e.type==='retirement'&&e.targetId==='K-PA.v2'));
