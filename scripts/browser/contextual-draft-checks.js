async page=>{
 ...
 try{
  await page.reload();const box=page.getByRole('textbox',{name:'Message Pathway',exact:true});await box.waitFor();const n=await page.locator('.agent-turn').count();await box.fill(cases.core[5].text);const start=Date.now();await box.press('Enter');await page.waitForFunction(n=>document.querySelectorAll('.agent-turn').length===n+1&&document.querySelector('#agent-form')?.getAttribute('aria-busy')==='false',n,{timeout:65000});const observedMs=Date.now()-start;
  const state=await page.evaluate(async()=>await(await fetch('/api/state')).json()),r=state.agent.entries.filter(e=>e.conversationId===state.agent.activeConversationId).at(-1);responses.push(r);latencies.push({query:r.query,submissionToAnswerMs:observedMs});
 ...
 return {schema:'pathway-contextual-draft-smoke-v1',timestamp:new Date().toISOString(),arm,environment:'Live correction check in existing isolated conversation; not the complete frozen comparison',results,responses,latencies,...(error?{error}:{})};
}
