...
if name not in ['conversation-checks.js','governance-checks.js','governance-continuation-checks.js','composer-visibility-checks.js','final-smoke-checks.js']: raise ValueError('Choose a repository browser check')
...
path.chmod(0o444)
passed=bool(report['results']) and all(r['pass'] for r in report['results']) and not report.get('error')
print(json.dumps({'artifact':str(path),'checks':len(report['results']),'passed':passed,'error':report.get('error')}))
